...
const PROJECT = 'vivir-fuengirola-negocios'
const BASE = '/paneladmin/vivir-en-fuengirola/negocios'
...
export default async function Page({
  ...
  return (
    <AdminShell
      title="Negocios — Vivir en Fuengirola"
      description="Empresas y negocios registrados en el portal"
      currentPath={BASE}
    ...
    >
      ...
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        >
          <span className="text-base leading-none">+</span> Nuevo negocio
        </Link>
      </div>
      ...
      {showNew && (
        <div className="mb-6 rounded-xl border border-line bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-bold text-foreground">{params.edit ? 'Editar negocio' : 'Nuevo negocio'}</h2>
      ...
      <Card className="overflow-hidden">
        <div className="overflow-x-auto">
          ...
          {data.clients.length === 0 && <p className="px-6 py-10 text-sm text-muted">No hay negocios todavía.</p>}
        </div>
      </Card>
    </AdminShell>
  )
